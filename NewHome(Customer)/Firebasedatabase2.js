...
//
//  CRITICAL FIX (v2):
//  SDK unified to 10.7.2 — MUST match CustomerBackend.js.
//  CustomerBackend.js owns Firebase Auth and uses 10.7.2.
//  ES modules are keyed by exact URL: two different SDK URLs
//  = two separate module bundles = two separate Firebase app
//  registries. getApps() on one bundle does NOT see the app
//  initialised by the other. initializeApp() fires twice,
//  creating a ghost app with no Auth state.
//  FIX: All three files (FirebaseDatabase, CustomerBackend,
//  FirebaseShop) now import from firebase/10.7.2 so they all
//  share the SAME single app instance via getApps()[0].
//
//  PURPOSE:
...
// ─── SDK 10.7.2 — MUST match CustomerBackend.js exactly ────
...
// ─────────────────────────────────────────
async function init() {
  console.log("[FirebaseDB] Initializing… SDK: 10.7.2");
  await seedDatabase();
