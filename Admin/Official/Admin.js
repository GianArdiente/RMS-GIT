/* ============================================================
   REV AUTO REPAIR — ADMIN DASHBOARD
   Part 2: JavaScript Backend (Admin Logic)
   ============================================================ */
...
function badge(s) {
  const m = {
    active: 'bg', inactive: 'br', pending: 'bd', confirmed: 'bb',
    completed: 'bg', cancelled: 'br', 'in-progress': 'bb', paid: 'bg',
    'in-stock': 'bg', low: 'bd', out: 'br', 'sold-out': 'br',
    available: 'bg', busy: 'bd', 'on-leave': 'bp',
    published: 'bg', draft: 'bd', basic: 'bd', standard: 'bb', full: 'bg'
  };
  return `<span class="b ${m[s] || 'bd'}">${s.charAt(0).toUpperCase() + s.slice(1).replace(/-/g,' ')}</span>`;
}

function fmt(p) {
  return parseFloat(p).toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}
  ...
  rows.forEach(r => {
    ...
    cols.forEach(c => {
      ...
      if (c === 'status') {
        ...
        td.style.cssText = 'font-family:Rajdhani,sans-serif;font-weight:700;font-size:11px;color:#A67F38;';
        td.textContent = v;
      } else if (c === 'amount' || c === 'cost') {
        td.style.cssText = 'font-family:"Barlow Condensed",sans-serif;font-weight:700;font-size:15px;color:#F2DB94;';
        td.textContent = c === 'amount' ? '₱' + v : v;
      } else {
        td.style.color    = '#888';
    ...
    if (act) {
      const td = document.createElement('td');
      td.innerHTML = `<button onclick='showBk(${JSON.stringify(r)})' class="btn-g" style="font-size:10px;padding:4px 11px;">View</button>`;
      tr.appendChild(td);
    }
    tb.appendChild(tr);
  });
...
/* ============================================================
   SECTION 5: JOB ORDERS
   ============================================================ */

function renderJO() {
  const tb = document.getElementById('joTb');
  if (!tb) return;
  tb.innerHTML = '';
  D.joborders.forEach(j => {
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:11px;color:#A67F38;">${j.id}</td>
      <td style="color:#999;font-size:12px;">${j.vehicle}</td>
      <td style="color:#777;font-size:12px;">${j.customer}</td>
      <td style="color:#777;font-size:12px;">${j.tech}</td>
      <td style="color:#666;font-size:12px;">${j.service}</td>
      <td style="min-width:90px;">
        <div style="display:flex;align-items:center;gap:7px;">
          <div class="prog-t" style="flex:1;"><div class="prog-f" style="width:${j.progress}%;"></div></div>
          <span style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:9px;color:#A67F38;">${j.progress}%</span>
        </div>
      </td>
...
function renderTechs() {
  ...
  D.techs.forEach((t, i) => {
    ...
    d.className = 'pc';
    d.innerHTML = `
      <div style="width:48px;height:48px;border-radius:50%;background:rgba(166,127,56,.1);border:1px solid rgba(166,127,56,.18);display:flex;align-items:center;justify-content:center;font-size:22px;margin:0 auto 9px;">${t.icon}</div>
      <div class="gt" style="font-family:'Barlow Condensed',sans-serif;font-size:15px;font-weight:800;margin-bottom:1px;">${t.name}</div>
      <div style="font-size:11px;color:#333;margin-bottom:1px;">${t.role}</div>
      <div style="font-size:10px;color:#222;margin-bottom:7px;">${t.exp} yrs · ★${t.rating}</div>
      <div style="margin-bottom:8px;">${badge(t.status)}</div>
      <div style="display:flex;gap:4px;">
        <button onclick="openEditPeople('tech',${i})" class="btn-e" style="flex:1;padding:5px;font-size:10px;"><i class="fas fa-pen"></i></button>
        <button onclick="openStatusEdit('tech',${i},'${t.name}')" class="btn-s" style="flex:1;padding:5px;font-size:10px;">Status</button>
        <button onclick="confirmDel('tech',${i})" class="btn-d" style="flex:1;padding:5px;font-size:10px;"><i class="fas fa-trash"></i></button>
      </div>`;
    g.appendChild(d);
  });
}
...
function renderStaffs() {
  ...
  D.staffs.forEach((s, i) => {
    ...
    d.className = 'pc';
    d.innerHTML = `
      <div style="width:48px;height:48px;border-radius:50%;background:rgba(166,127,56,.1);border:1px solid rgba(166,127,56,.18);display:flex;align-items:center;justify-content:center;font-size:22px;margin:0 auto 9px;">${s.icon}</div>
      <div class="gt" style="font-family:'Barlow Condensed',sans-serif;font-size:15px;font-weight:800;margin-bottom:1px;">${s.name}</div>
      <div style="font-size:11px;color:#333;margin-bottom:1px;">${s.role}</div>
      <div style="font-size:10px;color:#222;margin-bottom:7px;">${s.dept}</div>
      <div style="margin-bottom:8px;">${badge(s.status)}</div>
      <div style="display:flex;gap:4px;">
        <button onclick="openEditPeople('staff',${i})" class="btn-e" style="flex:1;padding:5px;font-size:10px;"><i class="fas fa-pen"></i></button>
        <button onclick="openStatusEdit('staff',${i},'${s.name}')" class="btn-s" style="flex:1;padding:5px;font-size:10px;">Status</button>
        <button onclick="confirmDel('staff',${i})" class="btn-d" style="flex:1;padding:5px;font-size:10px;"><i class="fas fa-trash"></i></button>
      </div>`;
    g.appendChild(d);
  });
}
...
/* ============================================================
   SECTION 7: PEOPLE EDIT (Techs / Staff)
   ============================================================ */
...
function openEditPeople(type, i) {
  const isT  = type === 'tech';
  const arr  = isT ? D.techs : D.staffs;
  const item = arr[i];
  peopleEditType = type;
  peopleEditIdx  = i;
...
/* ============================================================
   SECTION 8: POSTS (Featured / Promo / Shop)
   ============================================================ */

function openAddPost(type) {
  document.getElementById('postModalTitle').textContent = type === 'shop'  ? 'Add Shop Item'
    : type === 'promo' ? 'Add Promo'
    : type === 'tech'  ? 'Add Technician'
...
function prevPostImg(inp) {
  ...
  r.onload = e => {
    document.getElementById('postImgEl').src          = e.target.result;
    document.getElementById('postImgPrev').style.display = 'block';
    document.getElementById('postImgPh').style.display   = 'none';
  };
...
function savePost() {
  ...
  if (type === 'tech' || type === 'staff') {
    ...
    type === 'tech' ? renderTechs() : renderStaffs();
    return;
  }
  ...
  const obj = {
    ...
    stock:  parseInt(document.getElementById('postStock').value || 0),
    cat:    document.getElementById('postCat').value,
    type:   document.getElementById('postCat').value,
...
function openEditNews(i) {
  ...
  if (n.imgSrc) {
    document.getElementById('newsImgEl').src           = n.imgSrc;
    document.getElementById('newsImgPrev').style.display = 'block';
    document.getElementById('newsImgPh').style.display   = 'none';
  } else {
    document.getElementById('newsImgPrev').style.display = 'none';
    document.getElementById('newsImgPh').style.display   = 'block';
  }
...
function prevNewsImg(inp) {
  if (!inp.files[0]) return;
  const r = new FileReader();
  r.onload = e => {
    document.getElementById('newsImgEl').src           = e.target.result;
    document.getElementById('newsImgPrev').style.display = 'block';
    document.getElementById('newsImgPh').style.display   = 'none';
  };
  r.readAsDataURL(inp.files[0]);
...
function renderShopPage() {
  ...
  const items = D.shop.filter(s =>
    (shopFilter === 'All' || s.cat === shopFilter) &&
    s.name.toLowerCase().includes(shopSearch.toLowerCase())
  );

  if (!items.length) {
    g.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:40px;color:#333;">
      <i class="fas fa-box-open" style="font-size:32px;margin-bottom:10px;display:block;"></i>No items found</div>`;
    return;
  }
  ...
  items.forEach(s => {
    ...
    d.innerHTML = `
      ${s.imgSrc
        ? `<img src="${s.imgSrc}" class="post-img" alt="">`
        : `<div class="post-img-ph"><i class="fas fa-box" style="font-size:28px;color:#2a2a2a;"></i></div>`}
      <div class="post-body">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
          <span class="b bd" style="font-size:8px;">${s.cat}</span>${badge(s.status)}
        </div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
          ...
          <span style="font-size:10px;color:${s.stock === 0 ? '#f87171' : s.stock < 10 ? '#F2DB94' : '#10b981'};">
...
function filterShopCat(cat, btn) {
  shopFilter = cat;
  document.querySelectorAll('#shopFilterBtns .tab-btn').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  renderShopPage();
}

function renderShopDash() {
  const g = document.getElementById('shopGridDash');
  if (!g) return;
  g.innerHTML = '';
  D.shop.slice(0, 4).forEach((s, i) => {
    ...
    d.innerHTML = `
      ${s.imgSrc
        ? `<img src="${s.imgSrc}" style="width:100%;height:80px;object-fit:cover;display:block;" alt="">`
        : `<div style="width:100%;height:80px;background:linear-gradient(135deg,#1e1e1e,#141414);display:flex;align-items:center;justify-content:center;"><i class="fas fa-box" style="font-size:18px;color:#2a2a2a;"></i></div>`}
      <div style="padding:9px;">
        <div style="font-family:'Barlow Condensed',sans-serif;font-size:13px;font-weight:800;color:#ddd;margin-bottom:1px;">${s.name}</div>
        <div class="gt" style="font-family:'Barlow Condensed',sans-serif;font-size:16px;font-weight:900;">₱${fmt(s.price)}</div>
        <div style="font-size:9px;color:#3a3a3a;margin-top:2px;">${s.stock} in stock</div>
      </div>`;
    g.appendChild(d);
  });
}
...
function renderPromoList() {
  const g = document.getElementById('promoList');
  if (!g) return;
  g.innerHTML = '';
  if (!D.promos.length) {
    g.innerHTML = `<div style="color:#333;font-size:12px;padding:10px 0;">No promos yet.
      <button onclick="openAddPost('promo')" class="btn-g" style="font-size:10px;padding:3px 10px;margin-left:5px;">Add Promo</button></div>`;
    return;
  }
...
function renderNewsList() {
  const g = document.getElementById('newsList');
  if (!g) return;
  g.innerHTML = '';
  if (!D.news.length) {
    g.innerHTML = `<div style="color:#333;font-size:12px;padding:10px 0;">No news yet.
      <button onclick="openAddNews()" style="font-size:10px;padding:3px 10px;background:rgba(59,130,246,.1);border:1px solid rgba(59,130,246,.2);color:#60a5fa;border-radius:6px;cursor:pointer;margin-left:5px;">Add News</button></div>`;
    return;
  }
  D.news.forEach((n, i) => {
    const d = document.createElement('div');
    ...
    d.innerHTML = `
      ${n.imgSrc ? `<img src="${n.imgSrc}" class="news-img" alt="">` : `<div class="news-img-ph"><i class="fas fa-newspaper" style="font-size:28px;color:#1a1a3e;"></i></div>`}
      <div style="padding:13px;">
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:7px;">
          <span class="b bb" style="font-size:8px;">${n.category}</span>${badge(n.status)}
          <span style="font-size:9px;color:#2a2a2a;margin-left:auto;">${n.date || ''}</span>
        </div>
        ...
        <div style="display:flex;gap:5px;">
          <button onclick="openEditNews(${i})" class="btn-e" style="flex:1;padding:7px;font-size:11px;"><i class="fas fa-pen" style="margin-right:3px;"></i>Edit</button>
          <button onclick="confirmDel('news',${i})" class="btn-d" style="flex:1;padding:7px;font-size:11px;"><i class="fas fa-trash" style="margin-right:3px;"></i>Remove</button>
        </div>
      </div>`;
    g.appendChild(d);
  });
...
function renderPostCard(container, s, i, type) {
  ...
  d.innerHTML = `
    ${s.imgSrc
      ? `<img src="${s.imgSrc}" class="post-img" alt="">`
      : `<div class="post-img-ph"><i class="fas ${type === 'promo' ? 'fa-tag' : 'fa-star'}" style="font-size:26px;color:#2a2a2a;"></i></div>`}
    <div class="post-body">
...
function editPost(type, i) {
  ...
  if (s.imgSrc) {
    document.getElementById('postImgEl').src           = s.imgSrc;
    document.getElementById('postImgPrev').style.display = 'block';
    document.getElementById('postImgPh').style.display   = 'none';
  }
}
...
function renderFeatGrid() {
  ...
  D.featured.slice(0, 4).forEach((s, i) => {
    ...
    d.innerHTML = `
      ${s.imgSrc
        ? `<img src="${s.imgSrc}" style="width:100%;height:75px;object-fit:cover;display:block;" alt="">`
        : `<div style="width:100%;height:75px;background:linear-gradient(135deg,#1e1e1e,#141414);display:flex;align-items:center;justify-content:center;"><i class="fas fa-star" style="font-size:18px;color:#2a2a2a;"></i></div>`}
      <div style="padding:9px;">
        <div style="font-family:'Barlow Condensed',sans-serif;font-size:13px;font-weight:800;color:#ddd;margin-bottom:1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${s.name}</div>
        <div class="gt" style="font-family:'Barlow Condensed',sans-serif;font-size:16px;font-weight:900;">₱${fmt(s.price)}</div>
      </div>`;
    g.appendChild(d);
  });
...
function renderAccounts() {
  ...
  const tech   = D.accounts.filter(a => a.role === 'technician').length;
  const staff  = total - tech;
...
function openAddAccount() {
  ...
  ['accFname','accLname','accEmail','accPass','accPass2','accDept'].forEach(id => document.getElementById(id).value = '');
...
function saveAccount() {
  ...
  const obj  = {
    fname: fn, lname: ln, username: user, role,
    dept:   document.getElementById('accDept').value.trim() || 'General',
    access: document.getElementById('accAccess').value,
    status: 'active',
...
function openStatusEdit(type, idx, name) {
  ...
  (opts[type] || ['active','inactive']).forEach(o => {
    const op = document.createElement('option');
    op.value = o;
    op.textContent = o.charAt(0).toUpperCase() + o.slice(1).replace('-',' ');
    sel.appendChild(op);
  });
...
function saveStatus() {
  ...
  const idx  = parseInt(document.getElementById('stIdx').value);
  const val  = document.getElementById('stSel').value;
  const arr  = { tech: D.techs, staff: D.staffs, acc: D.accounts }[type];
  if (arr) arr[idx].status = val;
  closeModal('statusModal');
  if (type === 'tech')       renderTechs();
  else if (type === 'staff') renderStaffs();
  else                       renderAccounts();
}

/* ============================================================
   SECTION 14: DELETE CONFIRM
   ============================================================ */

function confirmDel(type, idx) {
  pendingDel = { type, idx };
  const msgs = {
    tech:     'Remove this technician from the team?',
    staff:    'Remove this staff member?',
    shop:     'Remove this shop item?',
    featured: 'Remove this featured service?',
    promo:    'Remove this promo?',
    news:     'Delete this news article?',
    acc:      'Delete this account? The user will lose access.'
  };
...
function doDelete() {
  ...
  const reMap = {
    tech:     renderTechs,
    staff:    renderStaffs,
    shop:     () => { renderShopPage(); renderShopDash(); },
    featured: () => { renderFeatGrid(); renderPosts(); },
    promo:    renderPosts,
    news:     renderPosts,
    acc:      renderAccounts
  };
...
function showBk(b) {
  ...
  document.getElementById('bkBody').innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
      ${[['Customer',b.customer],['Service',b.service],['Date',b.date],['Time',b.time],['Status',b.status]]
        .map(([k,v]) => `
          <div style="padding:10px;border-radius:9px;background:rgba(166,127,56,.04);border:1px solid rgba(166,127,56,.08);">
            <div style="font-size:9px;letter-spacing:2px;text-transform:uppercase;color:#282828;margin-bottom:4px;">${k}</div>
            <div style="font-size:13px;color:#ccc;">${k === 'Status' ? badge(v) : v}</div>
          </div>`).join('')}
    </div>`;
  openModal('bkModal');
}
...
/* ============================================================
   SECTION 16: DASHBOARD — RECENT BOOKINGS
   ============================================================ */
...
function renderRecent() {
  ...
  D.bookings.slice(0, 5).forEach(b => {
    ...
    d.innerHTML = `
      <div style="width:26px;height:26px;border-radius:6px;background:rgba(${c},.1);border:1px solid rgba(${c},.18);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
        <i class="fas ${icons[b.status] || 'fa-circle'}" style="font-size:9px;color:rgb(${c});"></i>
      </div>
      <div style="flex:1;min-width:0;">
        <div style="font-size:11px;font-weight:600;color:#ccc;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${b.customer}</div>
        <div style="font-size:9px;color:#2a2a2a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${b.service}</div>
      </div>
      ${badge(b.status)}`;
    d.onclick = () => showBk(b);
    el.appendChild(d);
...
/* ============================================================
   SECTION 17: DASHBOARD — REVENUE CHART
   ============================================================ */
...
function renderChart() {
  ...
  data.forEach((v, i) => {
    ...
    lbl.style.cssText = 'font-size:8px;font-family:Rajdhani,sans-serif;font-weight:700;color:#3a3a3a;';
    lbl.textContent   = v + 'k';
    const bar = document.createElement('div'); bar.className = 'chbar';
    bar.title         = v + 'k — Click to view';
...
/* ============================================================
   SECTION 18: DASHBOARD — LIVE FEED
   ============================================================ */

let livePtr = 0;

function renderLive() {
  const el = document.getElementById('liveEl');
  if (!el) return;
  el.innerHTML = '';
  ...
  items.forEach((it, i) => {
    ...
    d.innerHTML = `
      <div style="display:flex;align-items:center;gap:7px;">
        ...
        <span style="font-size:12px;color:#888;">${it.label}</span>
      </div>
      <span style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:9px;letter-spacing:1px;text-transform:uppercase;color:${it.color};">${it.status}</span>`;
    el.appendChild(d);
...
function renderNotifs() {
  ...
  const unr = D.notifs.filter(n => !n.read).length;
  b.textContent    = unr;
  b.style.display  = unr ? 'flex' : 'none';
  list.innerHTML   = '';

  D.notifs.forEach(n => {
    const d = document.createElement('div');
    d.className = 'nfi ' + (n.read ? 'rd' : 'unr');
    d.innerHTML = `
      <div style="width:37px;height:37px;border-radius:50%;background:${n.bg};display:flex;align-items:center;justify-content:center;flex-shrink:0;">
        <i class="fas ${n.icon}" style="color:${n.color};font-size:14px;"></i>
      </div>
      <div style="flex:1;">
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px;">
          <span style="font-family:Rajdhani,sans-serif;font-weight:800;font-size:9px;letter-spacing:1.5px;text-transform:uppercase;color:${n.read ? '#333' : '#A67F38'};">${n.tag}</span>
          ${!n.read ? '<span style="width:5px;height:5px;border-radius:50%;background:#F2DB94;display:inline-block;flex-shrink:0;"></span>' : ''}
        </div>
        <p style="font-size:12px;color:${n.read ? '#333' : '#ccc'};line-height:1.5;margin-bottom:3px;">${n.text}</p>
        <span style="font-size:9.5px;color:#222;">${n.time}</span>
      </div>`;
...
function toggleDD() {
  ddOpen = !ddOpen;
  document.getElementById('profDD').classList.toggle('open', ddOpen);
  document.getElementById('pChev').style.transform = ddOpen ? 'rotate(180deg)' : '';
}
function closeDD() {
  ddOpen = false;
...
document.addEventListener('click', e => {
  if (!document.getElementById('profWrap').contains(e.target)) closeDD();
});

/* ============================================================
   SECTION 21: SIDEBAR & RESPONSIVE
   ============================================================ */
...
function tick() {
  const now = new Date();
  const ct  = document.getElementById('clkTime');
...
/* ============================================================
   SECTION 23: REPORTS — DATA GENERATOR
   ============================================================ */
...
let weeklyDataGlobal = null, monthlyDataGlobal = null;
...
function reportToast(msg, type = 'ok') {
  const t = document.createElement('div');
...
/* ============================================================
   SECTION 24: REPORTS — HISTORY TABLE
   ============================================================ */
...
function buildHistoryRows(rows) {
  hFilt = [...rows].sort((a, b) => b.ts - a.ts);
  hPage = 1;
  const lbl = document.getElementById('historyCountLabel');
  if (lbl) lbl.textContent = hFilt.length + ' records';
  renderHPage();
  renderHPag();
}
...
function renderHPage() {
  ...
  if (!hFilt.length) {
    tb.innerHTML = `<tr><td colspan="9" class="px-3 py-12 text-center text-white/20 text-sm font-raj">No data found</td></tr>`;
    return;
  }
  tb.innerHTML = hFilt.slice((hPage - 1) * PS, hPage * PS).map(r => `
    <tr class="tr-hover border-b border-white/[.025]">
...
function _gP(p) {
  ...
  renderHPage();
  renderHPag();
  document.getElementById('historyTableWrapper').scrollTop = 0;
}

function applyDateFilter() {
  const dt = new Date(document.getElementById('dateInput').value);
  if (isNaN(dt.getTime())) { reportToast('Invalid date', 'err'); return; }

  const t = document.getElementById('dateFilterType').value;
  let s, e;
  if (t === 'day') {
    s = new Date(dt).setHours(0,0,0,0);
    e = new Date(dt).setHours(23,59,59,999);
  } else if (t === 'week') {
    const dy = dt.getDay();
    const sw = new Date(dt); sw.setDate(dt.getDate() - dy); sw.setHours(0,0,0,0);
    const ew = new Date(sw); ew.setDate(sw.getDate() + 6);  ew.setHours(23,59,59,999);
    s = sw.getTime(); e = ew.getTime();
  } else {
    s = new Date(dt.getFullYear(), dt.getMonth(), 1).getTime();
    e = new Date(dt.getFullYear(), dt.getMonth() + 1, 0, 23, 59, 59, 999).getTime();
  }
  buildHistoryRows(allRecords.filter(r => r.ts >= s && r.ts <= e));
  reportToast('Filter applied');
}

function clearDateFilter() {
  document.getElementById('dateInput').value = '';
  buildHistoryRows(allRecords);
  reportToast('Filter cleared');
}

/* ============================================================
   SECTION 25: REPORTS — EVENTS MODAL
   ============================================================ */
...
function buildEventsTable(evs) {
  ...
  tb.innerHTML = [...evs].sort((a,b) => b.ts - a.ts).map(ev => `
    <tr class="tr-hover border-b border-white/[.04]">
...
/* ============================================================
   SECTION 26: REPORTS — AGGREGATES
   ============================================================ */

function buildWeekly(readings) {
  const wk = Array.from({length:7}, () => ({b:0,j:0,w:0,o:0,r:0,c:0,n:0}));
  readings.forEach(r => {
    const i = new Date(r.ts).getDay();
    wk[i].b += r.bookings||0; wk[i].j += r.jobOrders||0; wk[i].w += r.walkIns||0;
    wk[i].o += r.onlineSales||0; wk[i].r += r.revenue||0; wk[i].c += r.cancellations||0; wk[i].n++;
  });
  const labels = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
  const dd = wk.map(x => x.n
    ? { bookings: Math.round(x.b/x.n), jobOrders: Math.round(x.j/x.n), walkIns: Math.round(x.w/x.n), onlineSales: Math.round(x.o/x.n), revenue: Math.round(x.r/x.n), cancellations: Math.round(x.c/x.n) }
    : { bookings:'--', jobOrders:'--', walkIns:'--', onlineSales:'--', revenue:'--', cancellations:'--' });
...
function buildMonthly(readings) {
  ...
  readings.forEach(r => {
    ...
    mo[m].b += r.bookings||0; mo[m].j += r.jobOrders||0; mo[m].w += r.walkIns||0;
    mo[m].o += r.onlineSales||0; mo[m].r += r.revenue||0; mo[m].c += r.cancellations||0; mo[m].n++;
  });
  const labels = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  const md = mo.map(m => m.n
    ? { bookings: Math.round(m.b/m.n), jobOrders: Math.round(m.j/m.n), walkIns: Math.round(m.w/m.n), onlineSales: Math.round(m.o/m.n), revenue: Math.round(m.r/m.n), cancellations: Math.round(m.c/m.n) }
    : { bookings:'--', jobOrders:'--', walkIns:'--', onlineSales:'--', revenue:'--', cancellations:'--' });
  return {
    labels, monthlyDetail: md,
    revBars:    mo.map(m => m.n ? Math.round(m.r/m.n) : 0),
    jobBars:    mo.map(m => m.n ? Math.round(m.j/m.n) : 0),
    bookBars:   mo.map(m => m.n ? Math.round(m.b/m.n) : 0),
    onlineBars: mo.map(m => m.n ? Math.round(m.o/m.n) : 0),
...
function setWBoxes(d) {
  ...
  Object.entries(M).forEach(([k, id]) => {
    ...
    const v  = d[k];
    el.textContent = v === '--' ? 'N/A' : (k === 'revenue' ? PHP(v) : v);
  });
}
...
/* ============================================================
   SECTION 27: REPORTS — YEAR STEPPERS
   ============================================================ */

function stepWeeklyYear(d) {
  weeklySelectedYear += d;
  document.getElementById('weeklyYearLabel').textContent = weeklySelectedYear;
  buildMonthSelForYear(weeklySelectedYear);
  populateWeeksForMonth();
  loadWeeklyData();
}

function stepMonthlyYear(d) {
  monthlySelectedYear += d;
  document.getElementById('monthlyYearLabel').textContent = monthlySelectedYear;
  loadMonthlyData();
}

/* ============================================================
   SECTION 28: REPORTS — WEEKLY CHART
   ============================================================ */
...
function renderWeeklyChart(data) {
  ...
  wcs.annotations.forEach(ann => datasets.push({
    label: ann.label, data: labels.map((_, i) => i === ann.dayIndex ? 9999 : null),
    borderColor:'#ff6b6b', borderDash:[3,3], borderWidth:2, pointRadius:0, fill:false, yAxisID:'y'
  }));
  ...
  if (activeCompare?.data) {
    ...
    datasets.push({
      ...
      data: cD, borderColor: metricColors[metric] || '#aaa',
      backgroundColor: 'transparent', borderDash:[7,4], borderWidth:2.5,
      tension:.4, spanGaps:true, pointRadius:5, pointStyle:'triangle',
      fill:false, yAxisID: isRevenue ? 'y1' : 'y'
    });
  }

  weeklyChart = new Chart(c, {
    type: 'line',
...
/* ============================================================
   SECTION 29: REPORTS — MONTHLY CHART
   ============================================================ */
...
function renderMonthlyChart(data) {
  ...
  ].map((ds, i) => ({ ...ds, borderWidth:1.5, borderRadius:6, borderSkipped:false, animation:{ delay: ctx => ctx.dataIndex * 30 + i * 90 } }));
...
/* ============================================================
   SECTION 30: REPORTS — LOAD DATA & SECTION SWITCH
   ============================================================ */
...
function loadMonthlyData() {
  if (monthlySelectedYear !== CUR_YEAR) {
    renderMonthlyChart({ hasData: false });
    ['bookings','jobOrders','walkIns','onlineSales','revenue','cancellations'].forEach(k => { const el = document.getElementById('mBox_' + k); if (el) el.textContent = 'N/A'; });
    return;
  }
...
function weeksInMonth(y, mi) {
  const f = new Date(y, mi, 1);
  return Math.ceil((f.getDay() + new Date(y, mi + 1, 0).getDate()) / 7);
}
...
function updateWeeklyDatesRow(startDate) {
  ...
  row.innerHTML = days.map((d, i) => {
    ...
    return `<div class="day-slot${isT ? ' active-day' : ''} flex flex-col items-center py-2 rounded-lg text-center"
      style="background:rgba(255,255,255,.02);border:1px solid rgba(166,127,56,.07)" onclick="selectDay(${i})">
...
function buildMonthSelForYear(year) {
  const sel = document.getElementById('monthlyMonthSelector');
  sel.innerHTML = '';
  for (let m = 0; m < 12; m++) {
    const o       = document.createElement('option');
    o.value       = `${year}-${String(m + 1).padStart(2,'0')}`;
    o.textContent = new Date(year, m, 1).toLocaleString(undefined, { month:'long' });
    sel.appendChild(o);
  }
...
function populateWeeksForMonth() {
  ...
  for (let i = 1; i <= Math.min(wks, 5); i++) {
    const o = document.createElement('option');
    o.value       = String(i);
    o.textContent = i === 5 ? 'Extra days' : `Week ${i}`;
    sel.appendChild(o);
  }
...
function switchSection() {
  ...
  if (v === 'weekly') {
    weeklySelectedYear = CUR_YEAR;
    document.getElementById('weeklyYearLabel').textContent = CUR_YEAR;
    initMonthSelector(); populateWeeksForMonth(); clearCompare(); loadWeeklyData();
  }
  if (v === 'monthly') {
    monthlySelectedYear = CUR_YEAR;
    document.getElementById('monthlyYearLabel').textContent = CUR_YEAR;
    loadMonthlyData();
  }
}

/* ============================================================
   SECTION 31: REPORTS — CHART CONTROLS
   ============================================================ */

function toggleTrends() {
  wcs.showTrendLines = !wcs.showTrendLines;
  document.getElementById('btnTrend').classList.toggle('active', wcs.showTrendLines);
  rebuildWeeklyChart();
  reportToast(wcs.showTrendLines ? 'Trend lines on' : 'Trend lines off');
}

function addAnnotation() {
  const di = prompt('Day to annotate (Sun, Mon, Tue, Wed, Thu, Fri, Sat):');
  if (!di) return;
  const ddays   = ['sun','mon','tue','wed','thu','fri','sat'];
  const dayIdx  = ddays.findIndex(d => d === di.toLowerCase().substring(0, 3));
  if (dayIdx === -1) { reportToast('Invalid day name', 'err'); return; }
  const lbl = prompt('Annotation label:');
  if (!lbl) return;
  wcs.annotations.push({ dayIndex: dayIdx, label: lbl });
  rebuildWeeklyChart();
  reportToast('Annotation added');
}

/* ============================================================
   SECTION 32: REPORTS — COMPARE MODAL
   ============================================================ */

const METRIC_LABELS = {
  bookings:'Bookings', jobOrders:'Job Orders', walkIns:'Walk-ins',
  onlineSales:'Online Sales', revenue:'Revenue (₱)', cancellations:'Cancels'
};
...
function openCompareModal() {
  if (weeklySelectedYear !== CUR_YEAR) { reportToast('No data for other years to compare', 'err'); return; }
  const ms = document.getElementById('monthlyMonthSelector')?.value; if (!ms) return;
  const [y, m] = ms.split('-').map(Number);
  ...
  for (let i = 1; i <= Math.min(wks, 5); i++) {
    ...
    card.onclick   = () => { document.querySelectorAll('.cmp-card').forEach(c => c.classList.remove('selected')); card.classList.add('selected'); compareSelectedWeek = i; };
  ...
  if (m > 1) {
    ...
    for (let i = 1; i <= Math.min(wksPrev, 4); i++) {
      ...
      const card = document.createElement('div');
      card.className   = 'cmp-card';
      card.dataset.month = String(pm + 1);
      card.innerHTML   = `<div class="wk-num" style="font-size:1.1rem">${prevMonthName} W${i}</div><div class="wk-dates">${lbl}</div>`;
      card.onclick     = () => { document.querySelectorAll('.cmp-card').forEach(c => c.classList.remove('selected')); card.classList.add('selected'); compareSelectedWeek = { week: i, month: pm + 1, year: y }; };
      grid.appendChild(card);
    }
  }
...
function applyCompare() {
  if (!compareSelectedWeek) { reportToast('Please select a week first', 'err'); return; }
  const ms = document.getElementById('monthlyMonthSelector')?.value; if (!ms) return;
  ...
  if (typeof compareSelectedWeek === 'number') { cmpM = curM - 1; cmpWk = compareSelectedWeek; }
  else { cmpM = compareSelectedWeek.month - 1; cmpWk = compareSelectedWeek.week; cmpY = compareSelectedWeek.year; }
  ...
  if (!cmpData.hasData) { reportToast('No data for that week', 'err'); closeCompareModal(); return; }
  ...
  closeCompareModal();
  rebuildWeeklyChart();
  reportToast(`Comparing ${METRIC_LABELS[metric]} with ${wkLabel}`);
}
...
document.addEventListener('keydown', e => {
  if (e.key === 'Escape') { closeLegend(); closeEventsModal(); closeCompareModal(); }
});

/* ============================================================
   SECTION 34: REPORTS — EXPORT
   ============================================================ */
...
document.addEventListener('click', e => {
  const p = document.getElementById('exportPanel');
  if (p.classList.contains('open') && !p.contains(e.target) && !e.target.closest('[onclick="openExportPanel()"]'))
    closeExportPanel();
});

function setExpProg(pct, msg) {
  document.getElementById('expProgFill').style.width = pct + '%';
...
async function startExport() {
  ...
  setExpProg(100, 'Done ✓');
  reportToast(type.toUpperCase() + ' ready');
  setTimeout(() => setExpProg(0, 'Ready'), 1800);
}

function exportExcel() {
  const wb = XLSX.utils.book_new();
  const ws = XLSX.utils.json_to_sheet(allRecords.map(r => ({
    Date: fmtDate(r.ts), Time: fmtTime(r.ts), Status: r.status,
    Bookings: r.bookings, 'Job Orders': r.jobOrders, 'Walk-ins': r.walkIns,
    'Online Sales': r.onlineSales, 'Revenue (PHP)': r.revenue, Cancellations: r.cancellations
  })));
  XLSX.utils.book_append_sheet(wb, ws, 'Daily_Records');

  if (document.getElementById('exp_inc_events').checked) {
    const ws2 = XLSX.utils.json_to_sheet(allEvents.map(ev => ({
      Date: fmtDate(ev.ts), Event: ev.label, Bookings: ev.bookings,
      'Job Orders': ev.jobOrders, 'Walk-ins': ev.walkIns, 'Online Sales': ev.onlineSales,
      'Revenue (PHP)': ev.revenue, Cancellations: ev.cancellations
    })));
    XLSX.utils.book_append_sheet(wb, ws2, 'Notable_Events');
  }

  const mD  = buildMonthly(allRecords);
  const ws3 = XLSX.utils.json_to_sheet(mD.labels.map((l, i) => {
    const d = mD.monthlyDetail[i];
    return { Month: l, Bookings: d.bookings, 'Job Orders': d.jobOrders, 'Walk-ins': d.walkIns, 'Online Sales': d.onlineSales, 'Avg Revenue': d.revenue, Cancellations: d.cancellations };
  }));
  ...
  a.download = `${fn}_${new Date().toISOString().slice(0,10)}.xlsx`;
  document.body.appendChild(a); a.click(); a.remove();
}

function exportPDF() {
  const { jsPDF } = window.jspdf || {};
  if (!jsPDF) { reportToast('jsPDF not available', 'err'); return; }
  const doc  = new jsPDF('p','mm','a4');
  const mg   = 14;
  let   y    = mg;
  const gold = [166,127,56];
  ...
  doc.autoTable({
    startY: y,
    head: [['Month','Bookings','Job Orders','Walk-ins','Online Sales','Avg Revenue (PHP)','Cancels']],
    body: mD.labels.map((l, i) => {
      const d = mD.monthlyDetail[i];
      return [l, d.bookings, d.jobOrders, d.walkIns, d.onlineSales, d.revenue === '--' ? '—' : PHP(d.revenue), d.cancellations];
    }),
    theme: 'grid', headStyles: { fillColor: gold }, styles: { fontSize: 8 }, margin: { left: mg, right: mg }
  });

  if (document.getElementById('exp_inc_events').checked && allEvents.length) {
    let y2 = doc.lastAutoTable.finalY + 10;
    if (y2 > 235) { doc.addPage(); y2 = mg; }
    doc.setFontSize(12); doc.setTextColor(0,0,0); doc.text('Notable Shop Events', mg, y2); y2 += 5;
    doc.autoTable({
      startY: y2,
      head: [['Date','Event','Bookings','Job Orders','Online Sales','Revenue (PHP)']],
      body: allEvents.map(ev => [fmtDate(ev.ts), ev.label, ev.bookings, ev.jobOrders, ev.onlineSales, PHP(ev.revenue)]),
      theme: 'striped', headStyles: { fillColor: gold }, styles: { fontSize: 8 }, margin: { left: mg, right: mg }
    });
  }
...
function svcHandleImg(p, e) {
  const file = e.target.files[0];
  if (!file) return;
  if (file.size > 5 * 1024 * 1024) { svcToast('Image must be under 5 MB','error'); return; }
  const r = new FileReader();
  r.onload = ev => { const b64 = ev.target.result; if (p === 'a') svcPendImgA = b64; else svcPendImgE = b64; svcSetImgPreview(p, b64); };
  r.readAsDataURL(file);
}

function svcSetImgPreview(p, src) {
  const zone = document.getElementById('svc' + p.toUpperCase() + 'ImgZone');
  if (!zone) return;
  zone.classList.add('has-preview');
  zone.innerHTML = `<img class="svc-img-preview" src="${src}" alt="preview"/>
    <button class="svc-img-clear-btn" onclick="svcClearImg('${p}')" title="Remove">
      <svg viewBox="0 0 24 24"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>
    </button>`;
}

function svcClearImg(p) {
  if (p === 'a') svcPendImgA = null; else svcPendImgE = null;
  svcResetImgZone(p);
}

function svcResetImgZone(p) {
  const zone = document.getElementById('svc' + p.toUpperCase() + 'ImgZone');
  if (!zone) return;
  zone.classList.remove('has-preview');
  zone.innerHTML = `<input type="file" id="svc${p.toUpperCase()}ImgInput" accept="image/*" onchange="svcHandleImg('${p}',event)"/>
...
function svcShowAddModal() {
  const p = document.getElementById('svcASkuPreview');
  if (p) p.textContent = svcGenerateSKU();
  svcPendImgA = null;
  svcResetImgZone('a');
  ['svcAName','svcAPrice'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
  document.getElementById('svcAStatus').value = 'active';
...
function svcRender() {
  ...
  tbody.innerHTML = slice.map(s => {
    ...
    const imgHtml = s.image ? `<img src="${s.image}" style="width:100%;height:100%;object-fit:cover;border-radius:7px;" alt="${s.name}"/>` : s.emoji;
    const chkCell = svcSelectMode ? `<td style="width:38px;padding:11px 6px 11px 16px;vertical-align:middle">
      <input type="checkbox" class="row-check"${isSel ? ' checked' : ''} onchange="svcToggleRowSelect('${s.id}',this)" onclick="event.stopPropagation()"/>
    </td>` : '';
    return `<tr class="${isSel ? 'selected' : ''}">${chkCell}
...
function svcRenderPag() {
  const pagEl = document.getElementById('svcPag'); if (!pagEl) return;
  ...
  for (let i = 1; i <= Math.min(pages, 5); i++) h += `<button class="svc-page-btn ${i === svcPg ? 'active' : ''}" onclick="svcGoPage(${i})">${i}</button>`;
...
function svcGoPage(p) {
  const mx = Math.ceil(svcFil.length / svcPp);
  if (p < 1 || p > mx) return;
  svcPg = p; svcRender();
}
...
function svcFilter() {
  ...
  svcFil  = svcData.filter(s => s.name.toLowerCase().includes(q) || s.id.toLowerCase().includes(q) || s.status.includes(q));
  svcPg   = 1; svcRender();
}
...
function svcSortBy(key) {
  if (svcSortKey === key) svcSortDir = svcSortDir === 'asc' ? 'desc' : 'asc';
  else { svcSortKey = key; svcSortDir = 'asc'; }
  ...
  svcFil.sort((a, b) => {
    ...
    return svcSortDir === 'asc' ? (va > vb ? 1 : -1) : (va < vb ? 1 : -1);
  });
  svcRender();
}
...
function svcView(id) {
  ...
  const hero = document.getElementById('svcVHero');
  const emojiEl = document.getElementById('svcVEmoji');
...
function svcOpenLightbox(src, caption) {
  document.getElementById('svcLightboxImg').src        = src;
  document.getElementById('svcLightboxCaption').textContent = caption || '';
  document.getElementById('svcLightbox').classList.add('open');
}
...
function svcGetCurrentPageIds() {
  const s = (svcPg - 1) * svcPp;
  return svcFil.slice(s, s + svcPp).map(s => s.id);
}
...
function svcOpenBulkEdit() {
  ...
  document.getElementById('svcBulkEditList').innerHTML = items.map(s =>
    `<div class="svc-bulk-list-item"><span style="font-size:16px">${s.emoji}</span><span class="svc-sku-badge">${s.id}</span><span style="flex:1;font-weight:500">${s.name}</span><span class="svc-price-tag" style="font-size:12px">$${s.price.toFixed(2)}</span><span class="svc-status-pill ${s.status}" style="font-size:9px"><span class="svc-status-dot"></span>${s.status}</span></div>`
  ).join('');
...
function svcConfirmBulkEdit() {
  ...
  let changed = 0;
  svcSelectedIds.forEach(id => {
    const i = svcData.findIndex(x => x.id === id); if (i === -1) return;
    if (newP  !== null) svcData[i].price  = newP;
    if (newSt !== '')   svcData[i].status = newSt;
    svcFil = svcFil.map(x => x.id === id ? svcData[i] : x);
    changed++;
  });
...
function svcOpenBulkDelete() {
  ...
  document.getElementById('svcBulkDelList').innerHTML = items.map(s =>
    `<div class="svc-bulk-del-item"><span style="font-size:16px">${s.emoji}</span><span class="svc-sku-badge">${s.id}</span><span style="flex:1;font-weight:500">${s.name}</span><span class="svc-price-tag" style="font-size:12px">$${s.price.toFixed(2)}</span></div>`
  ).join('');
...
function svcExportPDF() {
  ...
  doc.text('Services Report',margin,20); doc.text('Exported: '+svcFormatDate(),pageW-margin,15,{align:'right'}); doc.text('Total records: '+data.length,pageW-margin,20,{align:'right'});
  y = 34;

  const act   = data.filter(s=>s.status==='active').length;
  const total = data.reduce((a,s)=>a+s.price,0);
  ...
  data.forEach((s,idx) => {
    ...
    doc.setDrawColor(30,30,30); doc.setLineWidth(0.2); doc.line(margin,y+8,pageW-margin,y+8); y+=8;
  });
  ...
  for (let p = 1; p <= pageCount; p++) {
    ...
    doc.text('Rev Motors – Confidential',margin,fh-4); doc.text('Page '+p+' of '+pageCount,pageW-margin,fh-4,{align:'right'});
  }
...
function svcToast(msg, type) {
  type = type || 'info';
  const c = document.getElementById('svcToasts'); if (!c) return;
  const t = document.createElement('div');
...
function adjustStock(id, delta) {
  ...
  if (p.stock !== before) invToast(delta > 0 ? 'ok' : 'err', `${p.name}: ${before} → ${p.stock} units`);
  renderInventory();
}
    ...
    <tr class="${rowClass(p.stock)}">
      ...
      <td>
        <div class="inv-grp">
          ...
          <button class="inv-b-zero"       onclick="adjustStock(${p.id}, -${p.stock})" title="Zero out">⬇ 0</button>
        </div>
      </td>
    </tr>`).join('');
...
let _toastTimer;
function invToast(type, message) {
